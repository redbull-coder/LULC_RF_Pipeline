# -*- coding: utf-8 -*-
"""
横沥镇 2020-2025 年土地利用分类 - 循环版 v6（GDAL保存+边界裁剪+混淆矩阵）
"""
import arcpy
import os
import numpy as np
...
for year, paths in YEAR_CONFIG.items():
    ...
    try:
        ...
        present_classes = sorted([c for c in label_map.keys() if c in np.unique(y_valid)])
        labels = [label_map[c] for c in present_classes]
        cm = confusion_matrix(y_valid, y_pred, labels=present_classes)
        # 格式化混淆矩阵为对齐的文本表格
        header = "真实\\预测  " + "  ".join(f"{l:>6}" for l in labels)
        cm_lines = [header]
...
for year, oa, kappa in summary_results:
    if oa:
        ...
        print(f"  {year}: ❌ 失败")
summary_path = os.path.join(base_dir, "Summary_Accuracy_v6.txt")
